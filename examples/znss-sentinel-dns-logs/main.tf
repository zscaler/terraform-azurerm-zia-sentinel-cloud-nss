...
resource "azurerm_monitor_data_collection_rule" "data_collection_rule" {
  ...
  data_flow {
    ...
    transform_kql = local_file.dns_log_transform_kql.content
  }
  destinations {
    log_analytics {
...
locals {
  dns_log_kql = templatefile("${path.module}/transform_data/dns_logs_kql", {
  })
}

resource "local_file" "dns_log_transform_kql" {
  content  = local.dns_log_kql
  filename = "./transform_data/dns_logs.tpl"
}